...
void setup() {
  ...
  peak_interval = random(2000, 8000); // Random peak interval 2-8 seconds
  
  Serial.println("# PBT Simulator - Realistic Waveform Generation");
...
void generateRandomPeak() {
  // Generate random peak based on probabilities
  int rand_val = random(100);
  int cumulative = 0;
  
  for (int i = 0; i < PEAK_TYPES; i++) {
    cumulative += PEAK_PROBABILITIES[i];
  ...
  // Start peak generation
  generate_peak = true;
  peak_counter = PEAK_DURATION;
  
  Serial.print("# PEAK GENERATED: Amplitude=");
  Serial.print(peak_amplitude);
  Serial.print(" ADC (");
  Serial.print((peak_amplitude * 5.0) / 1023.0, 2);
  Serial.println("V)");
